...
var (
	...
	defaultDataDir              = filepath.Join("$HOME", ".rinkgo")
...
func addNodeFlags(fs *pflag.FlagSet) {
	...
	// Network ID
	fs.String(NetworkNameKey, constants.RinkName, "Network ID this node will connect to")
	...
	fs.Bool(NetworkAllowPrivateIPsKey, false, fmt.Sprintf("Allows the node to initiate outbound connection attempts to peers with private IPs. If the provided --%s is one of [%s, %s] the default is false. Oterhwise, the default is true", NetworkNameKey, constants.RinkName, constants.ChennaiName))
