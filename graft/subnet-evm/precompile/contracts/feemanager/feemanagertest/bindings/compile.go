...
//go:generate go run github.com/rink-labs/libevm/cmd/abigen --pkg bindings --type IFeeManager --abi ../../IFeeManager.abi --bin artifacts/IFeeManager.bin --out gen_ifeemanager_binding.go
//go:generate go run github.com/rink-labs/libevm/cmd/abigen --pkg bindings --type FeeManagerTest --abi artifacts/FeeManagerTest.abi --bin artifacts/FeeManagerTest.bin --out gen_feemanagertest_binding.go
...
//go:generate sh -c "sed -i.bak -e 's|github.com/rink-labs/libevm/accounts/abi|github.com/rink-labs/rinkgo/graft/subnet-evm/accounts/abi|g' -e 's|github.com/rink-labs/libevm/accounts/abi/bind|github.com/rink-labs/rinkgo/graft/subnet-evm/accounts/abi/bind|g' gen_ifeemanager_binding.go gen_feemanagertest_binding.go && rm -f gen_ifeemanager_binding.go.bak gen_feemanagertest_binding.go.bak"
