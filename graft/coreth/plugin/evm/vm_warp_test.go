...
import (
	...
	"github.com/rink-labs/rinkgo/graft/coreth/eth/tracers"
	...
	"github.com/rink-labs/rinkgo/graft/coreth/warp"
	...
	avagoUtils "github.com/rink-labs/rinkgo/utils"
...
func testWarpVMTransaction(t *testing.T, scheme string, unsignedMessage *avalancheWarp.UnsignedMessage, validSignature bool, txPayload []byte) {
	...
	warpBlock, err := vm.BuildBlockWithContext(t.Context(), blockCtx)
	require.NoError(err)
	...
	require.NoError(vm.SetPreference(t.Context(), warpBlock.ID()))
...
func testSignatureRequestsToVM(t *testing.T, scheme string) {
	...
	tests := []testCase{
		...
		},
		{
			...
			message: func() *avalancheWarp.UnsignedMessage {
				...
				require.NoError(t, err)
				msg, err := avalancheWarp.NewUnsignedMessage(vm.ctx.NetworkID, vm.ctx.ChainID, payload.Bytes())
				require.NoError(t, err)
				return msg
			}(),
	...
	for _, test := range tests {
		t.Run(test.name, func(t *testing.T) {
			...
			appErr := vm.Network.AppRequest(t.Context(), ids.GenerateTestNodeID(), 1, deadline, msg)
