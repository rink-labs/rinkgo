...
The Avalanche binary to be executed is named `rinkgo`.
...
The image should be tagged as `avaplatform/rinkGo:xxxxxxxx`, where `xxxxxxxx` is the shortened commit of the Avalanche source it was built from. To run the Avalanche node, run:
...
RinkGo is first and foremost a client for the Avalanche network. The versioning of RinkGo follows that of the Avalanche network.
