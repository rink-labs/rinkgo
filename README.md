...
If you plan to build RinkGo from source, you will also need the following software:
...
```sh
sudo su -
wget -qO -  | tee /etc/apt/trusted.gpg.d/rinkGo.asc
source /etc/os-release && echo "deb  $UBUNTU_CODENAME main" > /etc/apt/sources.list.d/rink.list
exit
```
...
The Rink binary to be executed is named `rinkgo`.
...
The image should be tagged as `avaplatform/rinkGo:xxxxxxxx`, where `xxxxxxxx` is the shortened commit of the Rink source it was built from. To run the Rink node, run:

```sh
docker run -ti -p 9650:9650 -p 9651:9651 avaplatform/rinkGo:xxxxxxxx /rinkGo/build/rinkGo
```
...
RinkGo uses multiple tools to generate efficient and boilerplate code.
...
RinkGo is first and foremost a client for the Rink network. The versioning of RinkGo follows that of the Rink network.
...
Because RinkGo's version denotes the network version, it is expected that interfaces exported by RinkGo's packages may change in `Patch` version updates.

### API Compatibility Guarantees

APIs exposed when running RinkGo will maintain backwards compatibility, unless the functionality is explicitly deprecated and announced when removed.
...
| RinkGo continuous integration | Tier 1  | Tier 2  | Tier 3  |
| ---------------------------------- | :-----: | :-----: | :-----: |
...
Please refer to our [Security Policy](SECURITY.md) and [Security Advisories](https://github.com/ava-labs/rinkGo/security/advisories).
